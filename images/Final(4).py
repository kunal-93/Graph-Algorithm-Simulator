...
import os,struct,pickle,gzip,math
...
class App(object):
    ...
    def __init__(self):
        ...
        self.statusbar.place(x=200,y=640,width=830,height=25)
        ...
        self.def_dist.place(x=42,y=640,width=155,height=26)
    ...
    def checkgraph(self):
        if(messagebox.askyesno(message="Do you want a Directed graph ? ",title="Graph Selection")):
           self.directed=1
    ...
    def initials(self):
        ...
        self.curved_edges=[[0 for x in range(1000)]for y in range(1000)]
        self.linedel=[]
        self.nodes=[]
        self.nodes_pos=[]
        self.count_entries=[]
    ...
    def addnode(self,event):
        ...
        for i in range(len(self.nodes_pos)):
            ...
            if(abs((pos[0]-check[0])**2-(pos[1]-check[1])**2)<750):
                g=1
                break
        if(len(self.nodes)==0):
            self.checkgraph()
    ...
    def checkedge(self,event):
        ...
        for i in range(len(self.nodes_pos)):
            ...
            if(abs((pos[0]-check[0])**2-(pos[1]-check[1])**2)<125):
                self.startx=pos[0]
                self.starty=pos[1]
                if(self.directed==1):
                    self.k=self.canvas.create_line(0,0,0,0,fill="coral",width=3,arrow="last",arrowshape=(16,20,6),joinstyle=ROUND,smooth=TRUE,capstyle=ROUND)
                elif(self.directed==0):
                    self.k=self.canvas.create_line(0,0,0,0,fill="coral",width=3,smooth=TRUE,joinstyle=ROUND)
    ...
    def onrelease(self,event):
        ...
        if(self.Eflag==1 and self.drawcheck==1):
            ...
            for i in range(len(self.nodes_pos)):
                ...
                if(int(abs((pos[0]-check[0])**2-(pos[1]-check[1])**2))<125 and self.checker1==1):
                    self.flag=1
                    break
            ...
            if(self.flag==1 and self.checker1==1):
                for x in range(len(self.nodes_pos)):
                    ...
                    if(abs((self.startx-check[0])**2-(self.starty-check[1])**2)<125):
                        p=1
                        break
                if(self.mat_node[x+1][i+1]>0 and self.checker==1 and self.checker1==1):
                    ...
                    pnt2=self.nodes_pos[i]
                    pnt1=self.nodes_pos[x]
                    x2=pnt2[0]
                    y2=pnt2[1]
                    x1=pnt1[0]
                    y1=pnt1[1]
                    ...
                    #print(theta)
                        if(x1<=x2 and y1>=y2):
                            pos1=(x1+15*math.cos(theta),y1-15*math.sin(theta))
                            pos2=(x2-15*math.cos(theta),y2+15*math.sin(theta))
                        ...
                        self.dis[self.cnt]=self.def_d
                        #self.dis[self.cnt].config
                        e=Entry(self.canvas,width=3,textvariable=self.dis[self.cnt],bg="tomato")

                        xc=(self.startx+event.x)/2
                        yc=(self.starty+event.y)/2
                        #p4=xc+yd  q4=yc-xd;
                        x1,y1=xc-yc+pos1[1],yc+xc-pos1[0];

                        if(self.mat_node[i+1][x+1]>0):
                            self.curved_edges[x+1][i+1]=z
                            self.canvas.coords(self.k,pos1[0],pos1[1],x1,y1,pos2[0],pos2[1])
                            e.place(x=(x1+xc)/2,y=(y1+yc)/2)
                        else:
                            self.canvas.coords(self.k,pos1[0],pos1[1],pos2[0],pos2[1])
                    ...
                    '''for i in range(1,len(self.nodes)+1):
                        ...
                        for j in range(1,len(self.nodes)+1):
                            print(self.curved_edges[i][j],sep=' ',end=' ')
                        print('\n')'''
    ...
    def checknode(self,event):
        ...
        for i in range(len(self.nodes_pos)):
            ...
            if(abs((pos[0]-check[0])**2-(pos[1]-check[1])**2)<70):
                self.snodenum=i
                break
        ...
        if(self.snodenum is not -1):
            ...
            if(self.directed==0):
                ...
                for k in range(1,x+1):
                    ...
                    if(n>0):
                        arr.append(n-1)
                        arr1.append(k-1)
                    n=self.mat_node[k][self.snodenum+1]
                    if(n>0):
                        darr.append(n-1)
                        darr1.append(k-1)
            ...
            for k in range(len(arr)):
                ...
                try:
                    ...
                    self.canvas.coords(emov,(pos1[0],pos1[1],pos2[0],pos2[1]))
                    enmov.place_forget()
                    x1=(prevpos[0]+pos[0])/2
                    y1=(prevpos[1]+pos[1])/2
                    enmov.place(x=x1,y=y1)
                except:
                    pass


            for k in range(len(darr)):
                prevpos=self.nodes_pos[darr1[k]]
                emov=self.edge_pos[darr[k]]
                enmov=self.count_entries[darr[k]]
                pnt2=pos
                pnt1=prevpos
                x2=pnt2[0]
                y2=pnt2[1]
    ...
    def relnode(self,event):
        if(self.snodenum is not -1):
            ...
            self.snodenum=-1
        lim=len(self.nodes)+1
        for i in range(1,lim):
            for j in range(1,lim):
                ep=self.curved_edges[i][j]
                if(ep>0):
                    pnt1,pnt2=self.nodes_pos[i-1],self.nodes_pos[j-1]
                    x2=pnt2[0]
                    y2=pnt2[1]
                    x1=pnt1[0]
                    y1=pnt1[1]
                    ...
                    yc=(pos1[1]+pos2[1])/2
                        #p4=xc+yd  q4=yc-xd;
                    x1,y1=xc-yc+pos1[1],yc+xc-pos1[0]
                    enmov=self.count_entries[ep-1]
                    emov=self.edge_pos[ep-1]
                    self.canvas.coords(emov,pos1[0],pos1[1],x1,y1,pos2[0],pos2[1])
                    enmov.place_forget()
                    enmov.place(x=(x1+xc)/2,y=(y1+yc)/2)
    ...
    def dist_entry(self,event=0):
        ...
        for i in range(y-1,-1,-1):
            ...
            self.count_entries[i].config(bg='tomato')
            self.coloredge.pop()
            ...
            for i in range(1,x+1):
                for j in range(1,x+1):
                    ...
                    if(i!=j and self.mat_node[i][j]==0):
                        self.mat_graph[i-1][j-1]=INF
                        if(self.directed==0):
                            self.mat_graph[j-1][i-1]=INF
                if(flag==1):
                    break
            if(flag==0):
                self.floyd_warshall()
    ...
    def color_path(self,source,destiny):
        ...
        for i in range(y-1):
            ...
            self.count_entries[x-1].config(bg="green")
...
\n    ** To check path and Distance between Two nodes provide source           node and desination node in the respective entry box."
    ...
    def op2(self,edg,nf,f3):
        ...
        for i in range(l1):
            ...
            x=self.canvas.create_line((pos1[0],pos1[1],pos2[0],pos2[1]),fill="coral",width=2,joinstyle=ROUND,smooth=TRUE,capstyle=ROUND)

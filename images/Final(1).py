    ...
    def checkedge(self,event):
        ...
        for i in range(len(self.nodes_pos)):
            ...
            if(abs((pos[0]-check[0])**2-(pos[1]-check[1])**2)<50):
                ...
                self.starty=pos[1]
                if(self.directed==1):
                    self.k=self.canvas.create_line(0,0,0,0,fill="coral",width=3,arrow="last")
                elif(self.directed==0):
                    self.k=self.canvas.create_line(0,0,0,0,fill="coral",width=3)
            ...
            if(self.flag==1 and self.checker1==1):
                ...
                if(self.mat_node[x+1][i+1]>0 and self.checker==1 and self.checker1==1):
                    ...
                    #print(theta)
                        ...
                        x1=(self.startx+event.x)/2
                        y1=(self.starty+event.y)/2
                        e.place(x=(self.startx+x1)/2,y=(self.starty+y1)/2)
                    except:
                        pass
...
\n    ** To check path and Distance between Two nodes provide source           node and desination node in the respective entry box."
    ...
    def op2(self,edg,nf,f3):
        ...
        for i in range(l1):
            ...
            x=self.canvas.create_line((pos1[0],pos1[1],pos2[0],pos2[1]),fill="coral",width=2)
